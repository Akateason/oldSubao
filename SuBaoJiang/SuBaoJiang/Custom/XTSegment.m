...
- (void)setup
{
    ...
    {
        XTPointButton *bt = [[XTPointButton alloc] init] ;
    ...
    // badges
    [self makeBadgesForSegmentsWithBadgeList:nil] ;
}

- (void)changeButtonSelectInfo
{
    for (UIView *subview in [self subviews]) {
        if ([subview isKindOfClass:[XTPointButton class]])
        {
            XTPointButton *bt = (XTPointButton *)subview ;
            bt.selected = NO ;
...
- (void)makeBadgesForSegmentsWithBadgeList:(NSArray *)badgeList
{
    for (UIView *subview in [self subviews])
    {
        ...
        {
            XTPointButton *bt = (XTPointButton *)subview ;
            int btIndex = (int)(bt.tag - TAGS_TEASEGMENT_BT) ;
            NSString *badgeStr = (!badgeList) ? @"0" : [NSString stringWithFormat:@"%@",badgeList[btIndex]] ;
            [self setBadgeWithbutton:bt badgeStr:badgeStr] ;
        }
    }
}

#pragma mark - badges set
- (void)setBadgeWithbutton:(XTPointButton *)button
                  badgeStr:(NSString *)str
{
    [button pointViewPosition] ;
    button.isMarked = [str integerValue] ;
}
