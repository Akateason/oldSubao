...
- (void)addShakeAnimationForView:(UIView *)view withDuration:(NSTimeInterval)duration
{
    ...
    if (randomNum >= 50)
    {
        ...
        animation.values = @[ @(currentTx), @(currentTx + 8), @(currentTx-5), @(currentTx + 5), @(currentTx -3), @(currentTx + 3), @(currentTx) ];
    ...
    else
    {
        ...
        animation.delegate = self;
        animation.duration = duration;
        animation.values = @[ @(currentTy), @(currentTy + 8), @(currentTy-5), @(currentTy + 5), @(currentTy -3), @(currentTy + 3), @(currentTy) ];
        animation.keyTimes = @[ @(0), @(0.225), @(0.425), @(0.6), @(0.75), @(0.875), @(1) ];
        animation.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut];
        [view.layer addAnimation:animation forKey:kAFViewShakerAnimationKey];
    }
