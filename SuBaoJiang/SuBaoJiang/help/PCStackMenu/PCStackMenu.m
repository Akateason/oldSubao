...
- (void)show:(PCStackMenuBlock)block
{
    ...
    [UIView animateWithDuration:0.2 animations:^{
		for(int i = 0; i < [_items count]; i++)
		{
			...
			stackItem.transform = CGAffineTransformMakeRotationAt(i * (_menuDirection == PCStackMenuDirectionClockWiseUp || _menuDirection == PCStackMenuDirectionClockWiseDown ? 2 : -2) * M_PI , point);
//            stackItem.transform = CGAffineTransformMakeRotationAt(i * (_menuDirection == PCStackMenuDirectionClockWiseUp || _menuDirection == PCStackMenuDirectionClockWiseDown ? 2 : -2) * M_PI / 90, point);
			stackItem.alpha = 1.0;
		}
				  ...
				  menuDirection:(PCStackMenuDirection)direction
{
	...
	for(int i = 0; i < max; i++)
	{
		CGRect r = CGRectMake((direction == PCStackMenuDirectionClockWiseUp || direction == PCStackMenuDirectionCounterClockWiseDown) ? 0 : startPoint.x,
							  ...
							  (direction == PCStackMenuDirectionClockWiseUp || direction == PCStackMenuDirectionCounterClockWiseDown) ? startPoint.x : self.frame.size.width - startPoint.x, itemHeight) ;
        //@ ADD BY TEA BEGIN
        r.origin.x += r.size.width * (i / 3) ;
        r.origin.y += r.size.height * (i / 3) * 3 ;
        //@ ADD BY TEA END
